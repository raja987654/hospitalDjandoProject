...
def contact(request):
    error = ""    # Récupération des données du formulaire
    if request.method == 'POST':
        ...
        try:
            # Création d'un nouvel objet Contact dans la base de données avec les données du formulaire
            Contact.objects.create(name=n, contact=c, email=e, subject=s, message=m, msgdate=date.today(), isread="no")
            error = "no"
        except:
            error = "yes" # Une erreur s'est produite lors de la création de l'objet Contact
...
def adminlogin(request):
    ...
    if request.method == 'POST':
        ...
        try:
            if user.is_staff:
                # Authentifie l'utilisateur en tant que membre du personnel/administrateur
                login(request, user)
                error = "no"
            else:
                error = "yes"
...
def add_doctor(request):
    ...
    if request.method == 'POST':
        ...
        profile_pic = request.FILES.get('profile_pic')  # Get the uploaded profile picture file
        try:
            # Create a new Doctor object with the data submitted in the form
            # and save the information in the database
            Doctor.objects.create(name=n, mobile=m, special=sp, profile_pic=profile_pic)
            error = "no"
        except:
            error = "yes"
